...
$logFile = __DIR__ . '/logs/debug_checkRemoteDisk.log';
$statusFile = __DIR__ . '/logs/disk_status_remote10.json';
$cronLogFile = '/var/log/cron_disk_remote10.log';
...
// Debug: log the parsed environment variables
// file_put_contents($logFile, '[' . date('Y-m-d H:i:s') . "] Loaded env from $envPath. Keys: " . implode(', ', array_keys($env)) . "\n", FILE_APPEND);

$remoteHost = $env['REMOTE_HOST'] ?? null;
$remoteUser = $env['REMOTE_USERNAME'] ?? null;
$remotePassword = $env['REMOTE_PASSWORD'] ?? null;

// file_put_contents($logFile, '[' . date('Y-m-d H:i:s') . "] REMOTE_HOST=$remoteHost, REMOTE_USERNAME=$remoteUser, REMOTE_PASSWORD=" . (empty($remotePassword) ? "EMPTY" : "SET") . "\n", FILE_APPEND);

if ($remoteHost === null || $remoteUser === null || $remotePassword === null) {
    file_put_contents($logFile, '[' . date('Y-m-d H:i:s') . "] Missing REMOTE_* values in .env\n", FILE_APPEND);
    file_put_contents($logFile, '[' . date('Y-m-d H:i:s') . "] Dumping all parsed env: " . json_encode($env, JSON_PRETTY_PRINT) . "\n", FILE_APPEND);
    exit(1);
}

$mountPoints = ['/root', '/data'];
...
$fromName = 'AFYACALL REMOTE .10 DISK ALERTS';
...
if (!empty($alerts)) {
    $body = "
        <div style='font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;'>
            ...
            <p style='font-size: 16px;'>The following mount points on .10 have crossed configured thresholds:</p>
    ...
    sendAlertEmail(
        '🚨 REMOTE .10 ALERT: Disk usage threshold crossed',
        $body,
        $toEmails,
        $fromName,
        $fromEmail,
...
if (!empty($recoveries)) {
    $body = "
        <div style='font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;'>
            ...
            <p style='font-size: 16px;'>The following mount points on .10 have returned to normal usage:</p>
            <ul style='font-size: 15px; color: #555;'>";
    ...
    sendAlertEmail(
        '✅ REMOTE .10 RECOVERY: Disk usage normalized',
        $body,
        $toEmails,
        $fromName,
        $fromEmail,
