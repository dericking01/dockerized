...
RUN echo "SHELL=/bin/bash\nPATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" > /etc/cron.d/cronjob \
 ...
 && echo '*/10 * * * * bash -c "/usr/local/bin/php /app/scripts/checkRemoteDisk.php >> /var/log/cron_disk_remote10.log 2>&1"' >> /etc/cron.d/cronjob \
 && echo '*/10 * * * * bash -c "/usr/local/bin/php /app/scripts/checkRemote11Disk.php >> /var/log/cron_disk_remote11.log 2>&1"' >> /etc/cron.d/cronjob \
...
#  && echo '59 * * * * bash -c "/usr/local/bin/php /app/scripts/send_revenue_alert.php >> /var/log/cron_revenue_sms.log 2>&1"' >> /etc/cron.d/cronjob \
