...
# CSV files to compare against
INPUT_FILES = [
    "/app/files/output/13_APR_2026_SMS_NSP_SOUTH.csv",
    "/app/files/output/october_base_clean.csv",
    "/app/files/output/14_APR_2026_SMS_NSP_NORTH.csv",
    "/app/files/output/14_APR_2026_IVR_NSP_NORTH.csv",
...
OUTPUT_FILE = "/app/files/output/15_APR_2026_DB_NOT_IN_FILES_ENTICEMENT_LOGS.csv"
...
def load_msisdns_from_files(file_paths: list[str]) -> Set[str]:
    ...
    for file_path in file_paths:
        ...
        logging.info("Loaded %d unique MSISDNs from %s", len(file_msisdns), file_path)
        all_msisdns.update(file_msisdns)

    logging.info("Total unique MSISDNs collected from all files: %d", len(all_msisdns))
    return all_msisdns
...
def fetch_db_msisdns() -> Set[str]:
    query = """
        ...
        WHERE created_at::date = %s
          AND (status = %s AND plan_code='921465_P02');
    """
...
def main() -> None:
    ...
    logging.info(
        "Starting inverse MSISDN comparison — DB present, files absent"
    )
    ...
    # Inverse: in DB but NOT in any of the files
    only_in_db = db_msisdns.difference(file_msisdns)
    logging.info(
        "MSISDNs present in DB but absent in all files: %d", len(only_in_db)
    )

    save_output(only_in_db, OUTPUT_FILE)
